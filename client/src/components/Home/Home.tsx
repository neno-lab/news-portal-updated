...
class Home extends React.Component<Props, HomeState> {
    ...
    addFavorites = (article:any) => {
        if(this.state.favorites.length===0){
            ...
            if(this.state.favorites.some((it:any)=> it.title.split(' ').join('')===article.title.split(' ').join(''))){
    ...
    render(){
        ...
        else if(this.state.selectedCategory==='hottest'){
            let filteredNews = this.props.news.filter((filter:any)=>{
                return filter.title.toLowerCase().includes(this.state.search.toLowerCase());
            });
            return(
                <> 
                ...
                <> 
                    ...
                    <div className='search-box'>
                        ...
                        <div className='favorites-wrapper'> {
                            this.state.favorites.map((it:any)=>(
                                <div key={Math.random().toString(36).substr(2,9)}>
                                    <div className='article-wrapper'><Article article={it}/></div>
                                </div>  
                            ))
                        }</div>
                    </div>
